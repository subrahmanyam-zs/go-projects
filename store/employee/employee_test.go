...
func TestCreateEmployee(t *testing.T) {
	testcases := []struct {
		...
		{"Invalid input", entities2.Employee{uid, "jason", "12-06-1999", "Bangalore", "CSE", 2}, true},
		{"valid input", entities2.Employee{uid, "jason", "12-06-1998", "Bangalore", "MBA", 1}, true},
	}
...
func TestUpdateEmployee(t *testing.T) {
	testcases := []struct {
		...
		{"Invalid input", uid, entities2.Employee{uid, "jason", "12-06-1999", "Bangalore", "CSE", 2}, true},
		{"", uid, entities2.Employee{uid, "jason", "12-06-1998", "Bangalore", "MBA", 1}, true},
	}
	var s Store
	db, mock, err := sqlmock.New()
	s = New(db)
...
func TestGetEmployee(t *testing.T) {
	testcases := []struct {
		...
		{"tcid id", uid, entities2.EmployeeAndDepartment{uid.String(), "jason", "12-06-1998", "Bangalore", "CSE", entities2.Department{2, "TECH", 2}}},
	}
	var s Store
	db, mock, err := sqlmock.New()
	s = New(db)
...
func TestGetAllWithCondition(t *testing.T) {
	...
	for i, tc := range testcases {
		...
		actualOutput := s.getAll(tc.name, tc.includeDepartment)
		if !reflect.DeepEqual(actualOutput, tc.expectedOutput) {
			t.Errorf("test case %v %s : Expected %v \nGot %v ", i+1, tc.desc, tc.expectedOutput, actualOutput)
		}
	}
...
func TestGetAllWithOutCondition(t *testing.T) {
	testcases := []struct {
		...
		{"no name and include department as true tcue", "", true, []entities2.EmployeeAndDepartment{{uid.String(), "jason", "12-06-1998", "Bangalore", "CSE", entities2.Department{2, "TECH", 2}}, {uid.String(), "vk", "05-11-1997", "Bangalore", "MBA", entities2.Department{1, "HR", 1}}}},
	}
...
func TestGetDepartment(t *testing.T) {
	...
	for i, tc := range testcases {
		...
		actualOutput := s.getDepartment(tc.input)
		if !reflect.DeepEqual(actualOutput, tc.expectedOutput) {
			t.Errorf("test case %v %s : Expected %v \nGot %v ", i+1, tc.desc, tc.expectedOutput, actualOutput)
		}
	}
