...
func TestK20220329122607_Up(t *testing.T) {
	mock, db := initializeTests(t)
	k := K20220329122607{}

	// register mock calls for success case
	mock.ExpectExec(DropPhone).WillReturnResult(sqlmock.NewResult(1, 0))
...
// nolint:dupl //Cannot use same testCase for migrateUP and migrateDOWN
func TestK20220329122607_Down(t *testing.T) {
	mock, db := initializeTests(t)
	k := K20220329122607{}

	// register mock calls for success case
	mock.ExpectExec(AddPhone).WillReturnResult(sqlmock.NewResult(1, 0))
	// register mock calls for failure case
	mock.ExpectExec(invalidQuery).WillReturnError(errors.New("invalid migration"))
