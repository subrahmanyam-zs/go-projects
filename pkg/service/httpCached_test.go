...
func TestCacheGetWithRedis(t *testing.T) {
	type resp struct {
	...
	c := cache.NewRedisCacher(redis)
	cacher := NewHTTPServiceWithOptions(ts.URL, log.NewMockLogger(b), &Options{Cache: &Cache{c, 0, nil}})
...
func (m mockCache) Set(key string, content []byte, duration time.Duration) error {
...
func TestCacheGetWithHeaders(t *testing.T) {
	...
	cacher := NewHTTPServiceWithOptions(ts.URL, log.NewMockLogger(b),
		&Options{Headers: map[string]string{"id": "1"}, Cache: &Cache{c, 0, nil}})
...
func Test_GetHeaders_All(t *testing.T) {
	expectedHeaders := map[string]string{"X-Correlation-Id": "123", "True-Client-IP": "127.0.0.1",
		"X-Authenticated-UserId": "990", "X-B3-TraceID": "123"}
