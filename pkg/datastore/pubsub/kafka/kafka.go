...
/*
	New establishes connection to Kafka
	using the config provided in KafkaConfig
*/
func New(config *Config, logger log.Logger) (*Kafka, error) {
	...
	if err != nil {
		return &Kafka{config: config}, err
	}
...
/*
	NewKafkaProducer returns a kafka producer object created
    using the configs provided. returns error if configs are invalid
*/
...
/*
	NewKafkaConsumer returns a kafka consumer object created
    using the configs provided. returns error if configs are invalid
*/
