...
func TestEventhub_HealthCheck(t *testing.T) {
	...
	testcases := []struct {
		...
		{Config{Namespace: "zsmisc-dev", EventhubName: "healthcheck",
			...
			Name: pkg.EventHub, Status: pkg.StatusUp, Host: "zsmisc-dev", Database: "healthcheck"}},
		{Config{Namespace: "host-name", EventhubName: "eventhub"},
			types.Health{Name: pkg.EventHub, Status: pkg.StatusDown, Host: "host-name", Database: "eventhub"}},
	}
