...
func Test_getEntry(t *testing.T) {
	tcs := []struct {
		...
		{"abc", "fake pass", true, LDAPOptions{CacheInvalidationFrequency: 10}, &CacheEntry{authorized: true}},
		{"xyz", "fake pass", true, LDAPOptions{Addr: ldapAddress}, &CacheEntry{}},
		{"xyz", "fake pass", false, LDAPOptions{Addr: ldapAddress}, &CacheEntry{}},
	}

	for i, tc := range tcs {
		l := NewLDAP(log.NewLogger(), &tc.options)
		key := getCacheKey(tc.user, tc.pass)

		if tc.cached {
			l.cache.cache[key] = tc.expected
		}

		got := l.getEntry(tc.user, tc.pass)
...
func Test_getEntryFromLDAPServer(t *testing.T) {
	tcs := []struct {
		...
		{"abc", "fake pass", CacheEntry{authorized: false}},
		{ldapUser, ldapPass, CacheEntry{authorized: true}},
	}
	l := NewLDAP(log.NewLogger(), &LDAPOptions{
		Addr:                       ldapAddress,
	...
	for i, tc := range tcs {
		result := l.getEntryFromLDAPServer(tc.user, tc.pass)
		assert.Equal(t, tc.expected.authorized, result.authorized, i)
	}
}
...
func TestLDAP(t *testing.T) {
	testcases := []struct {
		...
		{"missing headers", "order-service", "", "/hello", 401},
		{"exempt path", "order-service", "", "/.well-known/heartbeat", 200},
		{"valid token", "order-service", ldapToken, "/hello", 200},
	}
...
func TestLdap_Validate(t *testing.T) {
	...
	for i, testCase := range testCases {
		...
		err := l.Validate(logger, req)
		assert.Equal(t, testCase.err, err, i)
	}
}
